...
class User(AbstractUser):
    ...
    class Meta:#model中的内置类，用于处理默认信息
        verbose_name = '用户'#对应admin中管理的module块，显示的名字
        verbose_name_plural = verbose_name
        ordering = ['-id']

    def __unicode__(self):#调用自身的属性，默认输出一个对象的时候返回的信息
...
class Article(models.Model):
    ...
    objects = ArticleManager()#关联自定义manager
