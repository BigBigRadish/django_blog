...
from forms import LoginForm,CommentForm,RegForm
import json

logger = logging.getLogger('blog.views')#加载日志管理器

# Create your views here.
def global_setting(request):#初始化全局配置文件，一般在views中处理逻辑代码
    # 站点基本信息
...
def index(request):
    try:
        ...
        # 文章归档
        # 1、先要去获取到文章中有的 年份-月份  2018/01文章归档
...
def archive(request):
    try:
        ...
        article_list = Article.objects.filter(date_publish__icontains=year+'-'+month)#时间过滤，文章归档
        article_list = getPage(request, article_list)
    except Exception as e:
        logger.error(e)
...
def getPage(request, article_list):
    paginator = Paginator(article_list, 10)#分页的文章总数
    try:
        page = int(request.GET.get('page', 1))#得到当前页码
